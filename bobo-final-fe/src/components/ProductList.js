...
const ProductList = ({ products, onEdit, onDelete, userRole }) => {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "2rem auto", fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      <h2 style={{ color: "#264653", marginBottom: "1.5rem" }}>Product List</h2>
      <table
        style={{
          width: "100%",
      ...
      >
        <thead style={{ backgroundColor: "#1E40AF", color: "#ffffffff" }}>
          <tr>
            {["ID", "Product Name", "Description", "Price (€)", "Actions"].map((header) => (
              <th
                key={header}
                style={{
                  padding: "12px 15px",
        ...
        <tbody>
          {products.length === 0 ? (
            ...
            products.map((product) => (
              ...
              >
                <td style={{ padding: "10px 15px" }}>{product.id}</td>
                <td style={{ padding: "10px 15px" }}>{product.productName}</td>
                <td style={{ padding: "10px 15px" }}>{product.description}</td>
                <td style={{ padding: "10px 15px" }}>€ {product.unitPrice.toFixed(2)}</td>
